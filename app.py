...
if menu_opcao == "Visualizar Registros":
    st.header("📋 Banco de Dados Atual (Modo Administrator)" if st.session_state.role == "admin" else "📋 Banco de Dados Atual")
    ...
    if df.empty:
        ...
        with col_pdf:
            ...
            html_content = (
                ...
                f"<h2>Relatório de Solicitações PDCEAF - SRS Passos</h2>\n"
                f"<p><strong>Emitido por:</strong> {nome_usuario_atual}</p>\n"
                f"<p><strong>Nível de Acesso:</strong> {role_usuario_atual}</p>\n"
                f"<p><strong>Data de Exportação:</strong> {data_atual_pt}</p>\n"
                f"{html_table}\n</body>\n</html>"
            )
            st.download_button(
...
# --- ABA 3: GERENCIAR EXISTENTES (ORDEM DE TABULAÇÃO CORRIGIDA + MODAL DE DELEÇÃO) ---
    ...
    if df_edit.empty:
        ...
        with r2_c2:
            edit_num_sei = st.text_input("5. N° SE
